...
class CASAuthenticator < ::Auth::Authenticator
  ...
  def after_authenticate(auth_token)
    ...
    #
#	  Rails.logger.info "hello ***************************************> #{auth_token[:uid]} "
#	  Rails.logger.info "test: #{auth_token.inspect} "
#    person = People::Client::Person.find(auth_token[:uid])


    #account_name = generate_code(15)
    tmp = auth_token[:uid]
    account_name = tmp.gsub('@','')
    first_name = ''
    last_name = '' 
    ...
    user =
      User.where(username: account_name).first ||
      User.new(
    ...
    #user.update_column :bio_cooked, bio
